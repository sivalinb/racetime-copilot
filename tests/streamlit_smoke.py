...
at.text_input(key='import_title').set_value('Streamlit test - synthetic imported evidence')
at.text_area(key='import_text').set_value('WEBVTT\n\n00:00:10.000 --> 00:00:20.000\nSynthetic runner at the checkpoint.')
...
print('Streamlit AppTest passed: initial render, recap, conflicts, spoiler exclusion, approval, invalid cutoff, retry and transcript import.')
...
at.text_area(key='import_text').set_value(json.dumps([{'id':'live-1','start':60,'end':75,'text':'Synthetic runner at the start.','kind':'commentary'}]))
next(b for b in at.button if b.label == 'Import source').click().run()
...
at.text_area(key='import_text').set_value(json.dumps([{'id':'live-2','start':960,'end':975,'text':'Synthetic runner at the next checkpoint.','kind':'timing'}]))
...
print('Streamlit AppTest passed: live import, source selection, append, revision refresh, later-window recap, cache hit and current history.')
