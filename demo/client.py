...
class RaceTimeClient:
    def __init__(self, base_url: str | None = None):
        self.base_url = (
            base_url or os.environ.get("RACETIME_API_URL", "http://localhost:3000")
        ).rstrip("/")
    ...
    def call(self, path: str, body: dict | None = None) -> dict:
        try:
            ...
            raise RuntimeError(
                "The recap backend is unavailable. Start the demo with: python scripts/run_demo.py"
            ) from exc
        try:
            data = result.json()
        except ValueError as exc:
            raise RuntimeError(
                "The backend returned an unexpected response. Check its terminal output."
            ) from exc
        if not result.ok:
