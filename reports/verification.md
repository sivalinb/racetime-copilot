...
Not verified: independent real-video factual accuracy, active livestream ingestion, external LangSmith traces, independent human-reviewed dataset quality, public hosted deployment or real-user pilot benefit. One recorded YouTube interval passed integration as documented below. GitHub Actions results are separate from the local checks above.
...
- Earlier YouTube source `qnVos4_1soM`, interval 600–660 seconds: failed HTTP 500 with Gemini 3.5 and 3.8 Flash. Gemini 2.5 Flash returned HTTP 404 for this account. These historical failures remain recorded.
- Later direct YouTube source `S_9wb3g7jtY`, interval 300–480 seconds: **integration passed**, with seven observations, five cited recap sentences, no reported coverage gaps, `generated_verified` and status `awaiting_review`. No upload was needed. External tracing was disabled; factual accuracy and timestamps await independent review. [Full test report and reproduction](youtube-interval-test.md).
