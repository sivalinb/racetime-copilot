...
from demo.client import RaceTimeClient, clock, elapsed
...
def render_expansion_footer():
    ...
    st.image(
        str(ROOT / "public" / "art" / "expansion-footer.png"),
        caption="Five proposed extensions: sports, GPU test review, learning, incident handoffs and conferences.",
        width="stretch",
    )
    st.markdown(
    ...
    st.caption(
        "Future possibilities, not completed features. The scenes and sample screens are illustrative."
    )
    st.markdown(
        "[Explore the five use cases and what each needs](https://github.com/sivalinb/racetime-copilot/blob/main/docs/future-use-cases.md)."
    )
...
experience = st.sidebar.radio(
    "Workspace",
    ["Video workspace"]
    if os.getenv("RACETIME_VIDEO_ONLY") == "true"
    or os.getenv("RACETIME_PUBLIC") == "true"
    else ["Evidence demo", "Video workspace"],
    key="workspace_mode",
)
st.subheader("Why I’m building RaceTime")
st.caption(
    "I’m Siva—an ultrarunner, race organizer and crew member. When I miss a long race broadcast while sleeping or working, my mind stays with the race."
)
with st.expander("Meet Siva: the story behind RaceTime Copilot", expanded=False):
    founder_art = ROOT / "public" / "art" / "founder-story.png"
    st.image(
        str(founder_art),
        caption="An illustrated founder story, based on Siva’s photo. Scenes and app screen are illustrative.",
        width="stretch",
    )
    st.markdown(
        (ROOT / "docs" / "founder-story.md")
        .read_text()
        .replace(
            "(product-status.md)",
            "(https://github.com/sivalinb/racetime-copilot/blob/main/docs/product-status.md)",
        )
    )
    st.download_button(
        "Download founder story illustration",
        founder_art.read_bytes(),
        file_name="racetime-founder-story.png",
        mime="image/png",
...
st.caption(
    "For video or livestream questions, choose Video workspace. Add a link, set the time range, ask your question, then review the result."
)
with st.expander("Open the step-by-step guide and example questions"):
    st.markdown((ROOT / "docs" / "how-to-use.md").read_text())
with st.expander(
    "Recorded YouTube test: 05:00–08:00, result and trace", expanded=False
):
    ...
    st.download_button(
        "Download recorded YouTube test (JSON)",
        video_test.read_bytes(),
        file_name=video_test.name,
        mime="application/json",
        key="youtube_test_download",
    )
with st.expander("100 useful YouTube questions and scenarios", expanded=False):
    catalog_path = (
        ROOT / "evals-observability" / "datasets" / "youtube-question-scenarios-v1.json"
    )
    catalog = json.loads(catalog_path.read_text())
    st.caption(
        "100 suggested prompts, not 100 passed tests. The first 20 target the recorded 05:00–08:00 example; the other 80 need suitable footage. Set the time fields explicitly and replace placeholders before asking."
    )
    categories = list(dict.fromkeys(case["category"] for case in catalog["cases"]))
    category = st.selectbox(
        "Choose a question category", categories, key="youtube_question_category"
    )
    ...
    st.caption(
        "These prompts guide a short cited recap. Exact counts, exhaustive visual searches and complex formatting are not guaranteed. Drafts are not sent; timestamp suggestions do not create clips. Live questions require processed coverage."
    )
    st.download_button(
        "Download all 100 questions (Markdown)",
        (ROOT / "docs" / "youtube-question-library.md").read_text(),
        file_name="racetime-100-youtube-questions.md",
        mime="text/markdown",
        key="youtube_questions_md",
    )
    st.download_button(
        "Download scenario catalog (JSON)",
        catalog_path.read_bytes(),
        file_name=catalog_path.name,
        mime="application/json",
        key="youtube_questions_json",
    )
with st.expander(
...
with st.expander(
    ...
    st.caption(
        "Follow a race question through the illustrated overview, then explore the detailed components and decisions. Use the image fullscreen control or download a copy to zoom in."
    )
    illustrated_tab, system_tab, decision_tab = st.tabs(
        ["Illustrated overview", "System architecture", "Question lifecycle"]
    )
    with illustrated_tab:
        illustration = (
            ROOT / "public" / "architecture" / "racetime-illustrated-architecture.png"
        )
        st.image(
            str(illustration),
            caption="Illustrated architecture: input and queue → constrained agent → evidence and checks → human review. Sample race events are illustrative.",
            width="stretch",
        )
        st.download_button(
            "Download illustrated architecture (PNG)",
            illustration.read_bytes(),
            file_name=illustration.name,
            mime="image/png",
            key="illustrated_architecture",
        )
    for tab, filename, label in [
        ...
        with tab:
            st.caption(
                "Engineering plate: blue = application rules; amber = model operations; green = review/results; red = blocked or unverified."
            )
            diagram = ROOT / "public" / "architecture" / filename
            st.image(str(diagram), caption=label, width="stretch")
            st.download_button(
                "Download " + label.lower() + " (SVG)",
                diagram.read_bytes(),
                file_name=filename,
                mime="image/svg+xml",
                key=filename,
    ...
    st.caption(
        "Implementation and validation snapshot: 10 September 2026. The diagrams distinguish the video product from the separate evidence demo and LoRA lab."
    )
...
with left:
    ...
    st.write(
        "Catch up on any available race interval. Follow the evidence, inspect conflicting reports, and keep your spoiler boundary."
    )
with right:
    st.image(str(ROOT / "public/art/overview.png"), width="stretch")
st.info(
    "Evidence mode: import timestamped captions or observations. Use Video workspace for Gemini analysis. All Canyon Relay demo events are fictional."
)
try:
...
with st.sidebar:
    ...
    source_id = st.selectbox(
        "Evidence source",
        list(sources),
        format_func=lambda i: sources[i]["title"],
        key="source_select",
    )
    source = sources[source_id]
    st.caption(f"{source['kind'].title()} evidence · revision {source['revision']}")
    st.write(
        f"Available: **{clock(source['availableStart'])}–{clock(source['availableEnd'])}**"
    )
    ...
    st.caption(
        "Imports and reviews persist across reruns in this Streamlit session. Refreshing the page may start a new session."
    )
if st.session_state.get("active_source") != source_id:
    ...
    st.session_state["range_end"] = clock(
        min(source["availableEnd"], source["availableStart"] + 900)
    )
    st.session_state["range_cutoff"] = st.session_state["range_end"]
if notice := st.session_state.pop("notice", None):
    st.success(notice)
recap_tab, import_tab, history_tab, learning_tab = st.tabs(
    ["Recap", "Import / live append", "History & traces", "Capstone learning"]
)
with recap_tab:
    ...
    with controls:
        ...
        if source["provenance"] == "fictional_demo":
            st.success(
                "FICTIONAL REPLAY · Canyon Relay: two reports, one uncertain lead."
            )
        elif not source["url"]:
        ...
        with st.form("recap_form"):
            ...
            cutoff = st.text_input(
                ...
                key="range_cutoff",
                help="Must be at or after your interval end, within available coverage.",
            )
            question = st.text_input(
                "Your question", value="What happened?", max_chars=1000, key="question"
            )
            runner = st.text_input(
                "Runner filter (optional)", max_chars=120, key="runner_filter"
            )
            retry = st.checkbox(
                "Simulate one retrieval failure", key="simulate_failure"
            )
            submit = st.form_submit_button(
                "Create recap", type="primary", width="stretch"
            )
        if submit:
            try:
                with st.spinner("Retrieving and checking evidence…"):
                    st.session_state.run = api.call(
                        "recap",
                        {
                            "sourceId": source_id,
                            "start": elapsed(start),
    ...
    with results:
        ...
        if not run:
            ...
            st.write(
                "Your recap will show timestamped source reports, uncertainty and the execution trace."
            )
        else:
            ...
            for finding in run["findings"]:
                with st.container(border=True):
                    st.caption(
                        f"{clock(finding['start'])} · {finding['kind']} · {finding['id']}"
                    )
            ...
            for column, decision, label in [
                (approve, "approved", "Approve recap"),
                (reject, "rejected", "Reject recap"),
            ]:
                if column.button(label, key=decision, width="stretch"):
                    try:
                        st.session_state.run = api.call(
                            "review", {"id": run["id"], "decision": decision}
                        )["run"]
                        st.rerun()
                    except (ValueError, RuntimeError) as exc:
                        st.error(str(exc))
...
with import_tab:
    ...
    with st.form("import_form"):
        if not append:
            title = st.text_input(
                "Source title", value="My race broadcast", key="import_title"
            )
            url = st.text_input("YouTube URL (optional)", key="import_url")
            kind = st.selectbox("Source type", ["recorded", "live"], key="import_kind")
            duration = st.text_input(
                "Total duration / maximum live horizon",
                value="01:00:00",
                key="import_duration",
            )
            available_start = st.text_input(
                "Available start", value="00:00", key="import_start"
            )
        else:
            st.caption(
                f"Append to: {source['title']} · current revision {source['revision']}. Use new JSON evidence IDs."
            )
        available_end = st.text_input(
            "Available end",
            value=clock(source["availableEnd"]) if append else "15:00",
            key="import_end",
        )
        fmt = st.selectbox(
            "Evidence format", ["vtt", "srt", "json"], key="import_format"
        )
        ...
        import_submit = st.form_submit_button(
            "Append observations" if append else "Import source", type="primary"
        )
    if import_submit:
        try:
            content = (
                upload.getvalue().decode("utf-8-sig") if upload is not None else pasted
            )
            ...
            if append:
                body = {
                    ...
                    "availableEnd": elapsed(available_end),
                    "format": fmt,
                    "text": content,
                }
            else:
                body = {
                    "source": {
                        ...
                        "availableStart": elapsed(available_start),
                        "availableEnd": elapsed(available_end),
                    },
                    "format": fmt,
                    "text": content,
...
with history_tab:
    ...
    for old in workspace["history"]:
        with st.expander(
            f"{old['sourceTitle']} · {clock(old['start'])}–{clock(old['end'])} · {old['review']}"
        ):
