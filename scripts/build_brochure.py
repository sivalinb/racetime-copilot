...
def end():
 ...
 text('LOCAL CAPSTONE / Video + evidence workspaces / 10 SEP 2026',42,577,510,8,MUTED)
...
start('3 / How it works','One question. One evidence workflow.','Save a video, choose an interval and spoiler cutoff, then ask what happened. Review the cited evidence.')
art('workflow',170,210)
rows([('Retrieve','Keep only evidence fully inside the window.','Inspect bounded video windows and rank learned evidence vectors.'),('Check','Generate a cited recap and check its grounding.','Exclude later evidence. Say when there is not enough evidence.'),('Review','Read the timestamped recap and its trace.','Pause at a durable checkpoint; approve or reject, even after a restart.')],386,rh=53)
text('A bounded Gemini planner chooses legal actions. Coverage gaps and provider failures remain visible.',42,552,708,9,MUTED);end()
...
rows([('Week 1 / App','Streamlit, Python and persistent accounts.','Save videos, queue jobs and review results. TypeScript/Zod powers the optional evidence demo.'),('Week 2 / Retrieval','Gemini video, learned embeddings and citations.','Inspect a fixed interval, retrieve evidence, generate a recap and check the cited observations.'),('Week 3 / Workflow','LangGraph, SQLite jobs and durable review.','Bound model decisions, retry calls and resume review. Weighted LRU remains in the evidence demo.'),('Week 4 / Evaluation','Unit tests, AppTest, traces and human labels.','Test restart and time rules. Real-race labels and external trace quota are still required.'),('Week 5 / LoRA','PyTorch, Transformers, PEFT and scikit-learn.','Train, evaluate and merge a BERT-tiny adapter. This adapts the technique, not the exact Qwen3/LLaMA Factory setup.')],173,rh=73)
text('Live capture uses FFmpeg and yt-dlp. Public deployment scaffolding is prepared; the current scope is local.',42,550,708,9,MUTED);end()
...
for i,(value,label) in enumerate([('38 / 38','17 evidence + 21 product tests'),(f'{E["passed"]} / {E["count"]}','Evidence cases'),('Passed','Streamlit + API flows')]):
...
text('Real Gemini check: small uploaded video, embeddings, cited recap and durable review passed.',42,507,708,10)
text('YouTube and large-file processing remain provider-dependent. Active live capture and race accuracy need validation.',42,536,708,10,MUTED);end()

start('6 / Run it and extend it','A simple demo. A clear next step.','Video workspace: Python 3.11+ and a Gemini key. The optional evidence demo also needs Node.js 22.13+.')
box(42,175,340,190);text('START LOCALLY',58,188,308,11,BLUE,True)
commands=['python3 -m venv .venv','source .venv/bin/activate','pip install -r requirements.txt','# configure Gemini in .env','python scripts/run_product.py']
...
text('Create a local account. Save a short video and queue a question. Open Jobs / results, check timestamps and approve or reject the recap.',407,217,330,12)
text('For a key-free fallback, use run_demo.py and the fictional Canyon Relay scenario. Setup steps are in docs/setup.md.',407,302,330,10,MUTED)
rows([('Next','Validate real race clips and an active stream.','Review timestamps and claims independently. Resolve provider access and external trace quota.'),('Then','Pilot with race fans and organizers.','Measure time saved and evidence accuracy. Explore timing feeds, runner watchlists and multi-camera recaps.')],387,rh=62)
