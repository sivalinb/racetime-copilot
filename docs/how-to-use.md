...
Small MP4 uploads under 12 MB and a direct YouTube link have passed integration checks. On 10 September 2026, `S_9wb3g7jtY` at **05:00–08:00** produced seven observations and a cited recap without an upload. See the **Recorded YouTube test** panel in the app or [the saved report](https://github.com/sivalinb/racetime-copilot/blob/main/reports/youtube-interval-test.md). An earlier different URL and larger-file processing encountered provider errors. If a URL fails, try a short MP4 or switch to the key-free **Evidence demo** with the fictional Canyon Relay race. Real-race accuracy still needs independent review. The live workflow is implemented but its active-stream test is deferred.
